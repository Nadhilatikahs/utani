...
@section('contents')
    <h1 class="mb-0">Balai Penyuluhan Pertanian</h1>
    <hr />
    <form action="{{ route('bpps.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <fieldset disabled>
        ...
        <div class="row mb-3">
        <div class="col">
                <input type="hidden" name="id_bpp" class="form-control" placeholder="ID">
            </div>
        
        </div>
        <div class="row mb-3">
        <div class="col">
                <input type="text" name="nama_bpp" class="form-control" placeholder="Nama bpp" pattern="[A-Za-z\s]+" title="Hanya huruf yang diizinkan" required>
                @error('nama_bpp')
                <div id="flash-message" class="alert alert-danger">{{ $message }}</div>
                @enderror
            </div>
        <div class="col">
                <input type="text" name="alamat" class="form-control" placeholder="Alamat" reuired>
            </div>
            </div>
        <div class="row mb-3">
            <div class="col">
        ...
        <div class="row">
            <div class="col text-left d-flex gap-2">
                <a href="{{ route('bpps.index') }}" class="btn btn-secondary">Kembali</a>
