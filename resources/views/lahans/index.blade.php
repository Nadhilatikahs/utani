...
@section('contents')
    <div class="d-flex align-items-center justify-content-between">
        ...
        <a href="{{ route('lahans.create') }}" class="btn btn-primary">Tambah Data</a>
    </div>
    <hr />
    ...
    <table class="table table-hover">
        ...
        <tbody>+
                ...
                @forelse($lahans as $lh)
                    <tr>
                        <td class="align-middle">{{ $lh->kode_lahan }}</td>
                        <td class="align-middle">{{ $lh->nama_anggota }}</td>
                        <td class="align-middle">{{ format_uang($lh->luas,1, ',', '.')}} ha</td>
                        <td class="align-middle">{{ $lh->jml_petak }} petak</td>
                        ...
                        <td class="align-middle">
                            ...
                            <form action="{{ route('lahans.destroy', $lh->id_lahan) }}" method="POST" class="d-inline" id="delete-form-{{ $lh->id_lahan }}">
                                @csrf
                                @method('DELETE')
                                <button type="button" onclick="deleteConfirm({{ $lh->id_lahan }}, '{{ $lh->kode_lahan }}')" class="btn btn-danger btn-circle">
