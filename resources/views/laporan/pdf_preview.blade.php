...
<head>
    ...
    <style>
        ...
        /* ── Print preview button (hidden on print) ── */
        .print-bar {
            position: fixed; top: 0; left: 0; right: 0; z-index: 999;
            background: #1F6F54; color: white; padding: 10px 24px;
            display: flex; align-items: center; justify-content: space-between;
            font-family: 'Segoe UI', sans-serif; font-size: 14px;
        }
        .print-bar .pb-title { font-weight: 700; }
        .print-bar .pb-actions { display: flex; gap: 12px; }
        .btn-pb {
            background: white; color: #1F6F54; border: none;
            border-radius: 6px; padding: 7px 18px; font-weight: 700;
            font-size: 13px; cursor: pointer;
            text-decoration: none;
        }
        .btn-pb:hover { background: #e8f5e9; text-decoration: none;}
        .btn-pb-back {
        ...
        .btn-pb-back:hover { background: rgba(255,255,255,.2); color: white; text-decoration: none; }
        @media print {
            .print-bar { display: none; }
            body { padding-top: 0 !important; margin: 0; background: #fff;}
            .page { width: 100%; max-width: none; }
            .report-container { box-shadow: none; margin: 0; padding: 0; }
        }
        @media screen {
            body { padding-top: 52px; background: #f4f7f6; }
            .report-container { background: #fff; box-shadow: 0 0 10px rgba(0,0,0,0.1); margin-top: 20px; margin-bottom: 20px; padding: 40px; }
        }
    </style>
</head>
<body>
...
<div class="print-bar no-print">
    <div class="pb-title">Preview Laporan — {{ $biayaProduksi['tanam']['kode_tanam'] ?? $tanam->kode_tanam }}</div>
    <div class="pb-actions">
        <a href="{{ url()->previous() }}" class="btn-pb-back">← Kembali</a>
        <button class="btn-pb" onclick="window.print()">Download PDF</button>
    </div>
</div>
@php
